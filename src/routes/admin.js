...
router.post('/login', async (req, res) => {
  ...
  const matches = username === settings.adminUsername && await bcrypt.compare(password, adminPasswordHash);
  if (!matches) {
...
router.get('/', (_req, res) => {
  ...
  const latestInquiries = db.prepare(`SELECT i.*, a.name animal_name, d.name product_name
    FROM inquiries i LEFT JOIN animals a ON a.id=i.animal_id LEFT JOIN dairy_products d ON d.id=i.dairy_product_id
    ORDER BY i.created_at DESC LIMIT 6`).all();
  res.render('admin/dashboard', { title: 'Admin Dashboard', stats, latestInquiries });
});

router.get('/animals', (_req, res) => {
  res.render('admin/animals', {
    title: 'Manage Animals',
    animals: db.prepare('SELECT * FROM animals ORDER BY featured DESC, created_at DESC').all()
  });
});

router.get('/animals/new', (_req, res) => res.render('admin/animal-form', { title: 'Add Animal', animal: null, error: null }));
router.post('/animals', imageUpload.single('image'), (req, res) => {
  ...
  try {
    db.prepare(`INSERT INTO animals (name, slug, category, breed, age, gender, description, image, availability, featured)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(Object.values(data));
    return res.redirect('/admin/animals');
  } catch (error) {
    removeUpload(data.image);
    return res.status(400).render('admin/animal-form', {
      title: 'Add Animal', animal: { ...req.body, image: null },
      error: error.message.includes('UNIQUE') ? 'That slug is already in use.' : 'Unable to save animal.'
    });
  }
});
...
router.get('/animals/:id/edit', (req, res) => {
  ...
  return res.render('admin/animal-form', { title: 'Edit Animal', animal, error: null });
});
...
router.post('/animals/:id', imageUpload.single('image'), (req, res) => {
  ...
  if (!existing) { removeUpload(req.file && `animals/${req.file.filename}`); return res.redirect('/admin/animals'); }
  ...
  try {
    db.prepare(`UPDATE animals SET name=?, slug=?, category=?, breed=?, age=?, gender=?, description=?, image=?, availability=?, featured=? WHERE id=?`).run(data);
    if (req.file) removeUpload(existing.image);
    return res.redirect('/admin/animals');
  } catch (error) {
    if (req.file) removeUpload(image);
    return res.status(400).render('admin/animal-form', { title: 'Edit Animal', animal: { ...existing, ...req.body, image: existing.image }, error: error.message.includes('UNIQUE') ? 'That slug is already in use.' : 'Unable to update animal.' });
  }
});

router.post('/animals/:id/delete', (req, res) => {
  const animal = db.prepare('SELECT image FROM animals WHERE id=?').get(req.params.id);
  db.prepare('DELETE FROM animals WHERE id=?').run(req.params.id);
  removeUpload(animal?.image);
  res.redirect('/admin/animals');
});

router.get('/dairy', (_req, res) => res.render('admin/dairy', { title: 'Manage Dairy Products', products: db.prepare('SELECT * FROM dairy_products ORDER BY featured DESC, created_at DESC').all() }));
router.get('/dairy/new', (_req, res) => res.render('admin/dairy-form', { title: 'Add Dairy Product', product: null, error: null }));
router.post('/dairy', dairyUpload.single('image'), (req, res) => {
  const image = req.file ? `dairy/${req.file.filename}` : null;
  try {
    db.prepare('INSERT INTO dairy_products (name, slug, description, image, availability, featured) VALUES (?, ?, ?, ?, ?, ?)').run(
      safeText(req.body.name, 100), slugify(req.body.slug || req.body.name), safeText(req.body.description, 2000), image, safeText(req.body.availability, 30) || 'Available', bool(req.body.featured)
    );
    return res.redirect('/admin/dairy');
  } catch (error) {
    removeUpload(image);
    return res.status(400).render('admin/dairy-form', { title: 'Add Dairy Product', product: req.body, error: error.message.includes('UNIQUE') ? 'That slug is already in use.' : 'Unable to save product.' });
  }
});
router.get('/dairy/:id/edit', (req, res) => {
  const product = db.prepare('SELECT * FROM dairy_products WHERE id = ?').get(req.params.id);
  if (!product) return res.redirect('/admin/dairy');
  return res.render('admin/dairy-form', { title: 'Edit Dairy Product', product, error: null });
});
router.post('/dairy/:id', dairyUpload.single('image'), (req, res) => {
  const existing = db.prepare('SELECT * FROM dairy_products WHERE id=?').get(req.params.id);
  ...
  try {
    db.prepare('UPDATE dairy_products SET name=?, slug=?, description=?, image=?, availability=?, featured=? WHERE id=?').run(
      safeText(req.body.name, 100), slugify(req.body.slug || req.body.name), safeText(req.body.description, 2000), image, safeText(req.body.availability, 30) || 'Available', bool(req.body.featured), existing.id
    );
    if (req.file) removeUpload(existing.image);
    return res.redirect('/admin/dairy');
  } catch (error) {
    if (req.file) removeUpload(image);
    return res.status(400).render('admin/dairy-form', { title: 'Edit Dairy Product', product: { ...existing, ...req.body, image: existing.image }, error: error.message.includes('UNIQUE') ? 'That slug is already in use.' : 'Unable to update product.' });
  }
});
router.post('/dairy/:id/delete', (req, res) => {
  const product = db.prepare('SELECT image FROM dairy_products WHERE id=?').get(req.params.id);
  db.prepare('DELETE FROM dairy_products WHERE id=?').run(req.params.id);
  removeUpload(product?.image);
  res.redirect('/admin/dairy');
});

router.get('/gallery', (_req, res) => res.render('admin/gallery', { title: 'Manage Gallery', items: db.prepare('SELECT * FROM gallery_items ORDER BY featured DESC, created_at DESC').all() }));
router.post('/gallery', galleryUpload.single('image'), (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO gallery_items (title, category, image, description, featured) VALUES (?, ?, ?, ?, ?)').run(
      safeText(req.body.title, 120) || 'Farm Photo', safeText(req.body.category, 50) || 'Farm', image, safeText(req.body.description, 500), bool(req.body.featured)
    );
    return res.redirect('/admin/gallery');
  } catch (error) {
    removeUpload(image);
    return renderUploadError(res, 'Gallery', 'Unable to save gallery image.', {});
  }
});
router.post('/gallery/:id/delete', (req, res) => {
  const item = db.prepare('SELECT image FROM gallery_items WHERE id=?').get(req.params.id);
  db.prepare('DELETE FROM gallery_items WHERE id=?').run(req.params.id);
  removeUpload(item?.image);
  res.redirect('/admin/gallery');
});

router.get('/faqs', (_req, res) => res.render('admin/faqs', { title: 'Manage FAQs', faqs: db.prepare('SELECT * FROM faqs ORDER BY display_order ASC, id ASC').all() }));
router.post('/faqs', (req, res) => {
  ...
  if (question && answer) db.prepare('INSERT INTO faqs (question, answer, display_order, active) VALUES (?, ?, ?, ?)').run(question, answer, order, active);
...
router.get('/settings', (_req, res) => {
  res.render('admin/settings', { title: 'Website Settings', siteSettings: db.getSiteSettings(), error: null, success: null });
});
  ...
  try {
    for (const field of Object.keys(map)) {
      ...
      if (file) {
        ...
        newUploads.push({ relative, old: map[field] });
      }
    }
    const save = db.transaction(() => {
      for (const [key, value] of Object.entries(values)) db.setSiteSetting(key, value);
    });
    save();
    for (const item of newUploads) removeUpload(item.old);
    ...
    for (const item of newUploads) removeUpload(item.relative);
    console.error(error);
...
router.get('/inquiries', (_req, res) => {
  const inquiries = db.prepare(`SELECT i.*, a.name animal_name, d.name product_name
    FROM inquiries i LEFT JOIN animals a ON a.id=i.animal_id LEFT JOIN dairy_products d ON d.id=i.dairy_product_id
    ORDER BY i.created_at DESC`).all();
  res.render('admin/inquiries', { title: 'Customer Inquiries', inquiries });
});
router.post('/inquiries/:id/status', (req, res) => {
  const status = ['New', 'Contacted', 'Resolved'].includes(req.body.status) ? req.body.status : 'New';
